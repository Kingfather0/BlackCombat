...
const VERIFIED_TOTAL_OVERRIDE = {
  ...
  1246: {
    ...
    sellableTotal: 4653,
    noGradeTotal: 0,
    // 2026-08-20 "시야방해석" 소동 기록(총원은 결국 원래 값 그대로):
    //  - 17:41~17:50(KST): 29CM에 신규 등급 "시야방해석"(gradeCode 007, 정가 60,000원 등록)이
    //    생기고 기존 등급의 미판매 좌석 정확히 300석이 재분류됨(10분 주기 기록으로 1석 단위
    //    확인: 스탠다드 -155, 골드 -83, 선수 입장로 -62 = +300, 총 잔여 합계 불변).
    //    이때 그 300석은 좌석배치도 API에서도 통째로 빠져(누락 762→1,062석) 구매 불가 상태였다.
    //  - 같은 날 ~22:40: 300석이 전부 원래 등급으로 복귀(잔여 원상복구, 좌석배치도 누락도
    //    정확히 762석으로 복귀). 단, 007 등급 자체는 등급 목록·잔여 API에 0석짜리로 계속
    //    등록되어 남아있다 — 언제든 다시 활성화될 수 있는 상태이므로 지우지 않고 지켜본다.
    //  이 몇 시간 동안 총원을 재분류 기준(스탠 2,349/골드 391/입장로 254/시야 300)으로 바꿨다가,
    //  원복이 확인되어 아래 원래 실측값으로 되돌렸다. 다시 재분류가 일어나면(시야방해석 잔여가
    //  0보다 커지면) 그때의 이동량을 확인해 다시 반영해야 한다.
    totals: {
      '블랙티넘': 48,
      '골드': 474,
      '플로어': 214,
...
(async () => {
  ...
  try {
    ...
    for (const turn of turns) {
      ...
      for (const q of quantityList) {
        const gradeName = gradeDisplayName(q.seatGradeName, q.seatGradeCode);
        // quantityList에는 실제로 판매하지 않는 "유령" 등급이 잔여 0으로 섞여 나올 때가
        // 있습니다 (이 행사의 "시야방해석"이 그 예 — 실제 29CM 판매 페이지에는 뜨지도 않고
        // 가격 정보(seatGradePriceList)도 없는데, API의 quantityList에는 잔여 0으로 잡혀
        ...
        // 2026-08-20 보완: 단, 잔여가 0보다 크면 가격 정보가 없어도 기록합니다 — 이날 실제로
        // 기존 등급의 미판매 좌석 300석이 시야방해석으로 재분류돼 몇 시간 동안 잔여 300으로
